...
/**
 * CHU Y: gom xu ly loi vao 1 cho duy nhat (@RestControllerAdvice) thay vi try/catch rai rac trong
 * Controller. Frontend chi can bat HTTP status (400/422/500) + doc field "message"/"problems" de
 * hien thi, khong can quan tam exception Java cu the.
 */
...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<Object> handleParseError(CodeParseException ex, WebRequest request) {
    Object body =
        request.getDescription(false).contains("/api/ast/")
  ...
  public ResponseEntity<Map<String, Object>> handleUnexpected(Exception ex) {
    ...
    return ResponseEntity.internalServerError()
        .body(Map.of("message", "Da co loi khong mong muon xay ra o server"));
  }
}
