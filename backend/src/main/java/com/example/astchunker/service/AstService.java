...
public class AstService {
    ...
    public AstNode parse(String sourceCode) {
        ...
        throw new CodeParseException(
                "Unable to parse Java source code",
                "INVALID_JAVA_CODE",
                problems
        );
    }
    ...
    private AstNode convertSnippet(CompilationUnit compilationUnit) {
        ...
        compilationUnit.findFirst(MethodDeclaration.class, method -> "__parse".equals(method.getNameAsString()))
                .flatMap(MethodDeclaration::getBody)
                .map(BlockStmt::getStatements)
                .ifPresent(statements -> statements.forEach(statement -> root.addChild(astConverter.convert(statement))));
        return root;
    }

    private String formatProblem(Problem problem) {
        return problem.getLocation()
                .map(loc -> "Dong " + loc.toRange().map(r -> r.begin.line).orElse(-1) + ": " + problem.getMessage())
